...
from tests.conftest import TEST_TASKS
...
def test_endpoint_modify_task():
    ...
    response = client.put(
        "/tasks/2", json=updated_fields
    )
    assert response.status_code == 200
    assert response.json()["status"] == updated_fields["status"]
    response = client.put(
        "/tasks/3", json=updated_fields
    )
    assert response.status_code == 404
...
def test_endpoint_delete_task():
    ...
    assert response.status_code == 200
    expected_response = TEST_TASKS[1] # type: ignore
